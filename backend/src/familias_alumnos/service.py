...
def crear_alta_familia(
    ...
    log_audit(
        ...
        campo="__alta__",
        valor_anterior=None,
        valor_nuevo=f"{persona.nombre} {persona.apellido}",
...
def actualizar_familia(
    ...
    valores_anteriores = {campo: getattr(familia, campo) for campo in update_data}
...
def eliminar_familia(db: Session, familia: Familia, usuario_id: uuid.UUID | None = None) -> None:
    ...
    log_audit(
        db,
        entidad="FAMILIA",
        entidad_id=familia_id,
        campo="__eliminacion__",
        valor_anterior=str(persona_id),
        valor_nuevo=None,
        usuario_id=usuario_id,
    )
    db.commit()
...
def crear_alta_alumno(
    ...
    log_audit(
        ...
        campo="__alta__",
        valor_anterior=None,
        valor_nuevo=alumno.numero_legajo,
        usuario_id=usuario_id,
    )
    db.commit()
    db.refresh(persona)
...
def crear_alumno(
    ...
    log_audit(
        ...
        campo="__alta__",
        valor_anterior=None,
        valor_nuevo=nuevo_alumno.numero_legajo,
        usuario_id=usuario_id,
    )
    db.commit()
...
def obtener_o_crear_familia_para_persona_en_transaccion(
    db: Session, persona_id: uuid.UUID, usuario_id: uuid.UUID | None = None
) -> Familia:
...
def actualizar_alumno(
    ...
    `telefono`/`sexo` viven en la `Persona` vinculada (relación 1:1), no en `Alumno` — se
    aplican sobre `alumno.persona` en vez de `alumno`, pero se auditan bajo la misma entidad
    "ALUMNO" (con el nombre de campo `persona_<campo>`, igual que expone `AlumnoResponse`) para
    que aparezcan en el mismo historial que ve la ficha del alumno.
    """
    update_data = alumno_data.model_dump(exclude_unset=True)
    CAMPOS_PERSONA = {"telefono", "sexo"}
    ...
    valores_anteriores = {
        campo: getattr(alumno.persona if campo in CAMPOS_PERSONA else alumno, campo)
        for campo in update_data
    }

    for field, value in update_data.items():
        setattr(alumno.persona if field in CAMPOS_PERSONA else alumno, field, value)
    ...
    for campo, valor_nuevo in update_data.items():
        log_audit(
            ...
            entidad="ALUMNO",
            entidad_id=alumno.id,
            campo=f"persona_{campo}" if campo in CAMPOS_PERSONA else campo,
            valor_anterior=str(valores_anteriores[campo]) if valores_anteriores[campo] else None,
            valor_nuevo=str(valor_nuevo) if valor_nuevo is not None else None,
            usuario_id=usuario_id,
        )
...
def vincular_alumno_familia(
    ...
    log_audit(
        db,
        entidad="FAMILIA_ALUMNO",
        entidad_id=nuevo_vinculo.id,
        campo="__alta__",
        valor_anterior=None,
        valor_nuevo=f"familia={datos.familia_id} alumno={datos.alumno_id}",
        usuario_id=usuario_id,
    )
    db.commit()
...
def actualizar_vinculo(
    ...
    valores_anteriores = {campo: getattr(vinculo, campo) for campo in update_data}
...
def desvincular_alumno_familia(
    ...
    log_audit(
        ...
        campo="__eliminacion__",
        valor_anterior=f"familia={familia_id} alumno={alumno_id}",
        valor_nuevo=None,
        usuario_id=usuario_id,
    )
    db.commit()
