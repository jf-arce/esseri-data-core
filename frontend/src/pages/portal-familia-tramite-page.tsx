...
export function PortalFamiliaTramitePage({ titulo, descripcion }: PortalFamiliaTramitePageProps) {
  ...
  useEffect(() => { if (alumnoSeleccionado) listarAsistenciasFamilia(alumnoSeleccionado).then(setAsistencias).catch(() => setAsistencias([])) }, [alumnoSeleccionado])
  async function enviarJustificacion() {
    ...
    try {
      ...
      setAsistenciaAJustificar(null); setMotivo(''); setObservacion('')
  ...
  return (
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-5">
      ...
      <header>
        <p className="text-xs font-bold tracking-[.06em] text-texto-3 uppercase">Portal de familia</p>
        ...
        <CardContent>
          {esJustificacion && alumnos.length > 0 ? (
            <div className="flex flex-col gap-4">
              <label className="text-sm font-semibold">Alumno
                <select className="mt-1 block w-full rounded-lg border border-borde bg-superficie p-2" value={alumnoSeleccionado} onChange={(e) => setAlumnoId(e.target.value)}>
                  {alumnos.map((alumno) => <option key={alumno.alumno_id} value={alumno.alumno_id}>{alumno.nombre} {alumno.apellido}</option>)}
                </select>
              </label>
              <div className="divide-y divide-borde">
                {asistencias.map((asistencia) => <div key={asistencia.id} className="flex items-center gap-3 py-3 text-sm">
                  <span className="flex-1">{new Date(`${asistencia.fecha}T00:00:00`).toLocaleDateString('es-AR')}</span>
                  <span className="text-texto-2">{asistencia.tipo.replaceAll('_', ' ')}</span>
                  {asistencia.tipo === 'ausente_pendiente' && <Button size="sm" onClick={() => setAsistenciaAJustificar(asistencia.id)}>Justificar</Button>}
                  {asistencia.tipo === 'ausente_justificado' && <CheckIcon className="size-4 text-exito" aria-label="Justificada" />}
                </div>)}
              </div>
              {asistenciaAJustificar && <div className="rounded-card bg-fila-hover p-4">
                <p className="mb-3 text-sm font-semibold">Justificar ausencia</p>
                <label className="block text-sm">Motivo<input className="mt-1 w-full rounded-lg border border-borde bg-superficie p-2" value={motivo} onChange={(e) => setMotivo(e.target.value)} /></label>
                <label className="mt-3 block text-sm">Observación<textarea className="mt-1 w-full rounded-lg border border-borde bg-superficie p-2" value={observacion} onChange={(e) => setObservacion(e.target.value)} /></label>
                <div className="mt-3 flex gap-2"><Button variant="secondary" onClick={() => setAsistenciaAJustificar(null)}>Cancelar</Button><Button disabled={!motivo.trim() || guardando} onClick={() => void enviarJustificacion()}>Enviar justificación</Button></div>
              </div>}
            </div>
          ) : (
